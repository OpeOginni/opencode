...
describe("createPtyClient", () => {
  test("mints an authenticated ticket before opening the terminal socket", async () => {
    ...
    expect(request?.url).toBe(
      "https://server.example/base/api/pty/pty_1/connect-token?location%5Bdirectory%5D=%2Frepo%2Fworktree",
    )
    ...
    expect(socketURL?.toString()).toBe(
      "wss://server.example/base/api/pty/pty_1/connect?location%5Bdirectory%5D=%2Frepo%2Fworktree&cursor=42&ticket=ticket-1",
    )
...
describe("createPersistentPtyClient", () => {
  test("mints an authenticated ticket before opening the persistent terminal socket", async () => {
    ...
    expect(request?.method).toBe("POST")
    expect(request?.url).toBe("https://server.example/base/api/experimental/persistent-pty/pty_1/connect-token")
    expect(request?.headers.get("authorization")).toBe("Basic credential")
    expect(request?.headers.get("x-opencode-ticket")).toBe("1")
    expect(socketURL?.toString()).toBe(
      "wss://server.example/base/api/experimental/persistent-pty/pty_1/connect?ticket=persistent-ticket&cursor=42&attachment_id=attachment_1&takeover=true&input_protocol=1",
    )
    expect(socket.binaryType).toBe("arraybuffer")
  })
})
