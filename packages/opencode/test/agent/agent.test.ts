...
test("legacy agent tools keep top-level tool restrictions", async () => {
  await using tmp = await tmpdir({
    config: {
      permission: {
        bash: {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const build = await Agent.get("build")
      expect(PermissionNext.evaluate("bash", "ls", build!.permission).action).toBe("allow")
      expect(PermissionNext.evaluate("bash", "git status", build!.permission).action).toBe("deny")
    },
  })
})

test("explicit agent permission overrides inherited legacy tool restrictions", async () => {
  await using tmp = await tmpdir({
    config: {
      permission: {
        bash: {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const build = await Agent.get("build")
      expect(PermissionNext.evaluate("bash", "git status", build!.permission).action).toBe("allow")
    },
  })
})

test("legacy agent tools preserve inherited ask action at runtime", async () => {
  await using tmp = await tmpdir({
    config: {
      permission: {
        question: "ask",
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(evalPerm(review, "question")).toBe("ask")
    },
  })
})

test("legacy agent tools preserve inherited ask object rules at runtime", async () => {
  await using tmp = await tmpdir({
    config: {
      permission: {
        external_directory: {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const review = await Agent.get("review")
      expect(PermissionNext.evaluate("external_directory", "/other/path", review!.permission).action).toBe("ask")
      expect(PermissionNext.evaluate("external_directory", "/safe/project", review!.permission).action).toBe("allow")
    },
  })
})

test("legacy agent tools materialize allow when no root permission exists", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        review: {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const review = await Agent.get("review")
      expect(PermissionNext.evaluate("external_directory", "/other/path", review!.permission).action).toBe("allow")
      expect(evalPerm(review, "doom_loop")).toBe("allow")
    },
  })
})
