...
await $`docker buildx build --platform ${platforms} ${tagFlags} --push -f ./Dockerfile .`

const debianTags = [`${image}:debian-${version}`, `${image}:debian-${Script.channel}`, `${image}:debian`]
const debianFlags = debianTags.flatMap((t) => ["-t", t])
await $`docker buildx build --platform ${platforms} ${debianFlags} --push -f ./Dockerfile.debian .`
