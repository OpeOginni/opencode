...
Если вы используете модели, отличные от OpenAI, или видите ошибки, отдайте предпочтение провайдеру [Azure Cognitive Services](#azure-cognitive-services) с `@ai-sdk/openai-compatible` и `options.baseURL`; если вы в основном используете модели OpenAI, этот провайдер подойдет.
...
5. Задайте имя ресурса как переменную среды:
   ...
   Или переопределите его в конфигурации `opencode.json`:
...
:::note
Используйте конфигурацию переопределения ниже для моделей, отличных от OpenAI, или повторяющихся ошибок.
:::
...
Ключ API `/connect` все еще используется.

:::tip
Достигаете лимитов скорости? Уменьшите `models.<id>.limit.output`, чтобы оставаться ниже лимитов токенов в минуту Azure.
:::
