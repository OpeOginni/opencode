...
如果您使用非 OpenAI 模型或遇到错误，请优先使用 [Azure Cognitive Services](#azure-cognitive-services) 提供商，并配合 `@ai-sdk/openai-compatible` 和 `options.baseURL`；如果您主要使用 OpenAI 模型，则此提供商即可。
...
:::note
对于非 OpenAI 模型或反复出现的错误，请使用以下覆盖配置。
:::
...
:::tip
遇到速率限制？降低 `models.<id>.limit.output` 以保持在 Azure 每分钟令牌数上限之下。
:::
