...
Hvis du bruker ikke-OpenAI-modeller eller ser feil, foretrekk [Azure Cognitive Services](#azure-cognitive-services)-leverandøren med `@ai-sdk/openai-compatible` og `options.baseURL`; hvis du hovedsakelig bruker OpenAI-modeller, er denne leverandøren grei.
...
5. Angi ressursnavnet ditt som en miljøvariabel:
   ...
   Eller overstyr den i din `opencode.json`-konfigurasjon:
...
:::note
Bruk overstyringskonfigurasjonen nedenfor for ikke-OpenAI-modeller eller tilbakevendende feil.
:::
...
`/connect` API-nøkkelen brukes fortsatt.

:::tip
Når du hastighetsgrenser? Reduser `models.<id>.limit.output` for å holde deg under Azure token-per-minutt-grenser.
:::
