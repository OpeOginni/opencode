...
OpenAI olmayan modeller kullanıyorsanız veya hatalar görüyorsanız, `@ai-sdk/openai-compatible` ve `options.baseURL` ile [Azure Cognitive Services](#azure-cognitive-services) sağlayıcısını tercih edin; çoğunlukla OpenAI modelleri kullanıyorsanız, bu sağlayıcı iyidir.
...
5. Kaynak adınızı ortam değişkeni olarak ayarlayın:
   ...
   Veya `opencode.json` yapılandırmanızda geçersiz kılın:
...
:::note
OpenAI olmayan modeller veya tekrarlayan hatalar için aşağıdaki geçersiz kılma yapılandırmasını kullanın.
:::
      ...
      "models": {
        ...
        "claude-opus-4": {
          ...
          "provider": {
            ...
            "api": "https://<resource-name>.services.ai.azure.com/anthropic/v1"
          }
        }
      }
    }
...
`/connect` API anahtarı hala kullanılıyor.

:::tip
Hız sınırlarına mı ulaşıyorsunuz? Azure dakika başına token sınırlarının altında kalmak için `models.<id>.limit.output` değerini azaltın.
:::
