...
| `--continue` | `-c` | Son oturuma devam et                                                                          |
| `--session`  | `-s` | Devam edilecek oturum ID'si                                                                   |
| `--fork`     |      | Devam ederken oturumu çatalla (`--continue` veya `--session` ile kullanın)                    |
| `--password` | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| `--username` | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `opencode`) |
