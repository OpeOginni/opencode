...
| `--continue` | `-c` | 마지막 세션 이어서 실행                                                    |
| `--session`  | `-s` | 이어서 실행할 세션 ID                                                      |
| `--fork`     |      | 이어서 실행할 때 세션 포크 (`--continue` 또는 `--session`과 함께 사용)     |
| `--password` | `-p` | 기본 인증 비밀번호 (기본값: `OPENCODE_SERVER_PASSWORD`)                    |
| `--username` | `-u` | 기본 인증 사용자 이름 (기본값: `OPENCODE_SERVER_USERNAME` 또는 `opencode`) |
