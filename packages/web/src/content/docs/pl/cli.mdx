...
| `--continue` | `-c`  | Kontynuuj ostatnią sesję                                                                              |
| `--session`  | `-s`  | Identyfikator sesji do kontynuowania                                                                  |
| `--fork`     |       | Rozgałęź sesję podczas kontynuowania (użyj z `--continue` lub `--session`)                            |
| `--password` | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| `--username` | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `opencode`) |
