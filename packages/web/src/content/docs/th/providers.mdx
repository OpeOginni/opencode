...
หากคุณใช้โมเดลที่ไม่ใช่ OpenAI หรือพบข้อผิดพลาด ให้เลือกผู้ให้บริการ [Azure Cognitive Services](#azure-cognitive-services) ที่มี `@ai-sdk/openai-compatible` และ `options.baseURL` หากคุณใช้โมเดล OpenAI เป็นหลัก ผู้ให้บริการรายนี้ก็ใช้ได้
...
5. ตั้งชื่อทรัพยากรของคุณเป็นตัวแปรสภาพแวดล้อม:
   ...
   หรือแทนที่ในการกำหนดค่า `opencode.json` ของคุณ:
...
:::note
ใช้การกำหนดค่าแทนที่ด้านล่างสำหรับโมเดลที่ไม่ใช่ OpenAI หรือข้อผิดพลาดที่เกิดซ้ำ
:::
...
คีย์ API `/connect` ยังคงถูกใช้งานอยู่

:::tip
ถึงขีดจำกัดอัตราแล้วใช่ไหม? ลด `models.<id>.limit.output` เพื่อให้อยู่ภายใต้ขีดจำกัดโทเค็นต่อนาทีของ Azure
:::
