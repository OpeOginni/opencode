...
如果您使用非 OpenAI 模型或看到錯誤，請優先使用 [Azure Cognitive Services](#azure-cognitive-services) 提供商，並搭配 `@ai-sdk/openai-compatible` 和 `options.baseURL`；如果您主要使用 OpenAI 模型，則此提供商即可。
...
:::note
對於非 OpenAI 模型或反覆出現的錯誤，請使用以下覆蓋設定。
:::
...
:::tip
遇到速率限制？降低 `models.<id>.limit.output` 以保持在 Azure 每分鐘權杖上限之下。
:::
