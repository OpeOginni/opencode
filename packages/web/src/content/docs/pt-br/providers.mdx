...
Se você usar modelos que não sejam da OpenAI ou encontrar erros, prefira o provedor [Azure Cognitive Services](#azure-cognitive-services) com `@ai-sdk/openai-compatible` e `options.baseURL`; se você usar principalmente modelos da OpenAI, este provedor está bem.
...
5. Defina o nome do seu recurso como uma variável de ambiente:
   ...
   Ou substitua na sua configuração `opencode.json`:
...
:::note
Use a configuração de substituição abaixo para modelos que não sejam da OpenAI ou erros recorrentes.
:::
...
A chave da API `/connect` ainda é usada.

:::tip
Atingindo limites de taxa? Reduza `models.<id>.limit.output` para ficar abaixo dos limites de token por minuto do Azure.
:::
